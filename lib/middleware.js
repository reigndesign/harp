 ...
 * Redirector
 *
 * Redirects certain requests by using the 'redirects' hash in _harp.json
 *
 */
exports.redirector = function(req, res, next) {
  if(req.setup.config.hasOwnProperty("redirects") && req.setup.config["redirects"] instanceof Object) {
    var redirects = req.setup.config["redirects"];
    
    var pathn = parse(req).pathname;
    if (redirects[pathn]) {
      var dest = redirects[pathn];
      console.log("Redirecting from "+pathn+" to "+dest);
      res.statusCode = 301;
